...
public class ParameterUtil {
	...
	public static Object conver(Object value, Class toClazz){
		Object result = null;
			...
			if(toClazz == Date.class){
				try{
					...
					try{
						...
						result = null;
						log.error("parameter type cast exception.", ex);
					}
				}
			}else{
				DefaultTypeConverter converter = new DefaultTypeConverter();
				result = converter.convertValue(null, value, toClazz);
			}
        }  
		...
		for (PropertyDescriptor descriptor : propertyDescriptors) {
			...
			if (map.containsKey(propertyName)) {  
				...
				if(values != null) { 
					...
					value = ParameterUtil.conver(value, targetClass);
