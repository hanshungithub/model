...
public class ObjectUtil {
    ...
    /**
     * <p>如果对象为<code>null</code>，则返回指定默认对象，否则返回对象本身。</p>
     * <p>create by jin</p>
     * @param t 要测试的对象
     * @param defaultValue 默认值
     * @param <T> 测试对象的类型
...
//    }
    ...
    /**
     * 判断传入的参数中，是否有null
     * @param objects 要监测的对象
     * @return 如果 objects 中有一个为 null 则返回 true
     */
    ...
    public static void isNullThrowz(BOExceptionEnum exception, Object... objects){
        if (objects.length <= 0) {
            return ;
        }
        if (isNull(objects)) {
            BoException.throwz(exception);
        }
    }

    public static void isNullDefaultThrowz(Object... objects) {
        isNullThrowz(BOExceptionEnum.SYS_ERROR, objects);
    }

    public static void assertNotNull(Object... objects) {
        isNullDefaultThrowz(objects);
    }
    ...
    public static <T> boolean or(T o,T... compares){
        if (compares.length <= 0) {
            throw new IllegalArgumentException();
        }
        for (T compare : compares) {
            if(Objects.equals(o, compare)){
                return true;
            }
        }
        return false;
    ...
    public static <T> boolean and(T o,T... compares){
        if (compares.length <= 0) {
            throw new IllegalArgumentException();
        }
        for (T compare : compares) {
